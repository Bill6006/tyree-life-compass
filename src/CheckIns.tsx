...
export function CheckIns({ database, records, draft, loaded, error: storageError, journal, onEditingChange }: Props) {
  ...
  async function begin(record?: CheckIn) {
    ...
    setBusy(true);
    const next = newDraft(record?.block ?? block, record);
  ...
  if (session) return <CheckInFlow key={session.id} database={database} initial={session} onClose={() => setSession(null)} onSaved={(record) => { setSession(null); setSelectedId(record.id); setMessage(record.reportedAt === record.updatedAt ? 'Saved on this device.' : 'Correction saved. Comparisons have been recalculated.'); window.scrollTo({ top: 0, behavior: 'instant' }); }} />;
  return <>
    ...
    {!loaded ? <p role="status">Opening your readings…</p> : <>
      ...
      {selected ? <>
        ...
        <RecordView record={selected} history={records} onEdit={() => void begin(selected)} onDelete={() => setDeleteId(selected.id)} />
