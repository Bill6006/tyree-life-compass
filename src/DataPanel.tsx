...
function download(content: string, type: string, extension: string) {
  ...
  anchor.href = url; anchor.download = `life-compass-export-${new Date().toISOString().slice(0, 10)}.${extension}`;
...
export function DataPanel({ database, records, draft }: { database: AppDatabase; records: CheckIn[]; draft: Draft | null }) {
  ...
  async function restore() {
    ...
    setBusy(true);
    try {
      const result = await restoreBackup(database, backup);
  ...
  function exportFile(extension: 'json' | 'csv') {
    const data = makeBackup(records, draft);
