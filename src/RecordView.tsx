...
export function RecordView({ record, history, onEdit, onDelete }: { record: CheckIn; history: CheckIn[]; onEdit: () => void; onDelete: () => void }) {
  ...
  return <>
    ...
    <details className="recorded-panel"><summary>Recorded · your phrases</summary><dl className="phrase-list">{questionsFor(record.block).map((id) => <div key={id}><dt>{readings[id].label}{coreIds.includes(id) ? ' · score ingredient' : ''}</dt><dd>{record.answers[id] === undefined ? 'Not logged yet' : readings[id].anchors[record.answers[id]!]}</dd></div>)}</dl><p className="secondary">Reported {formatTime(record.reportedAt)}{record.updatedAt !== record.reportedAt ? ` · Corrected ${formatTime(record.updatedAt)}` : ''}.</p><p className="secondary">Active answering time: {Math.round(record.answeringMs / 1000)} seconds. Closed time and pauses beyond 30 seconds between interactions are excluded.</p></details>
